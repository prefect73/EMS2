...
<head>
	...
	<script type="text/javascript">
        ...
        function poulateAvailableHours(){
            ...
            var userTD ='<td><select class="form-control input-sm userCombo" name="workPackageUserAllocations['+index+'].user"><c:forEach items="${employeeslist}" var="emp"><c:if test="${emp.active}"><option class="form-control input-sm" value="${emp.id}">${emp.firstName}</option> </c:if></c:forEach> </select></td>';
